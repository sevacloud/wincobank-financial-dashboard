...
const { fyStart, fyEnd, isAdmin } = window.wincobankData || {};
...
function PriorYearCell( { fy, code, value, onSaved } ) {
            ...
            <td style={ { padding: '4px 8px' } }>
                <div style={ { display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'flex-end' } }>
                    ...
                    <button
                        onClick={ handleSave }
                        disabled={ saving }
                        title={ __( 'Save', 'wincobank-dashboard' ) }
                    ...
                    <button
                        ...
                        title={ __( 'Cancel', 'wincobank-dashboard' ) }
    ...
    return (
        <td>
            <button
                onClick={ startEdit }
                title={ __( 'Click to edit prior year figure', 'wincobank-dashboard' ) }
    ...
    return (
        <div className="wb-table-wrap">
            <table className="wb-table">
                <thead>
                    <tr>
                        <th style={ { width: 68 } }>{ __( 'Code', 'wincobank-dashboard' ) }</th>
                            ...
                            <th key={ k }>
                                ...
                                { data[ k ]?._error && (
                                    <span style={ { fontWeight: 400, fontSize: '.7rem', display: 'block', color: 'rgba(255,255,255,.6)' } }>
                                        { __( '(unavailable)', 'wincobank-dashboard' ) }
                                    </span>
                                ) }
                            </th>
                        ) ) }
                        ...
                        <th>
                            ...
                            <span style={ { display: 'block', fontWeight: 400, fontSize: '.75rem' } }>
                                £{ isAdmin && <em style={ { color: 'rgba(255,255,255,.65)', marginLeft: 4 } }>{ __( '(click to edit)', 'wincobank-dashboard' ) }</em> }
                            </span>
                        </th>
                    </tr>
                </thead>
                ...
                <tbody>
                    { navyRow( __( 'Income and Endowments', 'wincobank-dashboard' ) ) }
                    { renderSection( incomeRows ) }
                    { renderSubtotal( __( 'Total Income and Endowments', 'wincobank-dashboard' ), incomeRows ) }

                    { navyRow( __( 'Expenditure', 'wincobank-dashboard' ) ) }
                    { renderSection( expendRows ) }
                    { renderSubtotal( __( 'Total Expenditure', 'wincobank-dashboard' ), expendRows ) }
                </tbody>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td style={ { fontWeight: 700, color: 'var(--navy)' } }>
                            { __( 'Net Income / (Expenditure)', 'wincobank-dashboard' ) }
                        </td>
                        { ACCOUNT_KEYS.map( ( k ) => {
                            const v = netAcct( k );
                            return (
    ...
    return (
        <div>
            ...
            <p style={ { fontSize: '.8125rem', color: 'var(--muted)', marginBottom: 16 } }>
                { __( 'Statement of Financial Activities (Charity SORP).', 'wincobank-dashboard' ) }
                { ' ' }{ __( 'Period:', 'wincobank-dashboard' ) } { params.from } { __( 'to', 'wincobank-dashboard' ) } { params.to }
                { isAdmin && <em>{ ' — ' }{ __( 'Prior year figures: click any cell to edit.', 'wincobank-dashboard' ) }</em> }
            </p>
            ...
            { ! loading && data && (
                <div className="wb-card">
                    <h3 className="wb-card__title">
                        { __( 'Statement of Financial Activities', 'wincobank-dashboard' ) }
