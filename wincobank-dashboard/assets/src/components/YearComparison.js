...
const { currentYear } = window.wincobankData || {};
    ...
    const renderSection = ( label, rows ) => (
        <>
            ...
            <tr style={ { background: 'var(--bg)' } }>
                <td style={ { fontWeight: 700 } }>{ __( 'Subtotal', 'wincobank-dashboard' ) }</td>
                { years.map( ( y ) => (
    ...
    return (
        <div>
            <div className="wb-year-inputs">
                <label htmlFor="wb-y1">{ __( 'Year 1 (start)', 'wincobank-dashboard' ) }</label>
                <input id="wb-y1" type="number" min="2000" max={ cur } value={ year1 } onChange={ ( e ) => setYear1( Number( e.target.value ) ) } />
                <label htmlFor="wb-y2">{ __( 'Year 2 (start)', 'wincobank-dashboard' ) }</label>
                <input id="wb-y2" type="number" min="2000" max={ cur } value={ year2 } onChange={ ( e ) => setYear2( Number( e.target.value ) ) } />
                <label>{ __( 'Year 3 (current):', 'wincobank-dashboard' ) } <strong>{ fyLabel( cur ) }</strong> { __( '(auto)', 'wincobank-dashboard' ) }</label>
                <button className="wb-btn" onClick={ fetchData } disabled={ loading }>
                    { loading ? __( 'Loading…', 'wincobank-dashboard' ) : __( 'Load Comparison', 'wincobank-dashboard' ) }
                </button>
            </div>
            ...
            { ! loading && data && (
                <div className="wb-card">
                    <h3 className="wb-card__title">{ __( '3-Year Income & Expenditure Comparison', 'wincobank-dashboard' ) }</h3>
                    <div className="wb-table-wrap">
                        <table className="wb-table">
                            <thead>
                                <tr>
                                    <th>{ __( 'Nominal Code / Description', 'wincobank-dashboard' ) }</th>
                                    { years.map( ( y ) => <th key={ y }>{ fyLabel( y ) }</th> ) }
                                </tr>
                            </thead>
                            <tbody>
                                { renderSection( __( 'Income', 'wincobank-dashboard' ), sectionRows( 'Income' ) ) }
                                { renderSection( __( 'Expenditure', 'wincobank-dashboard' ), sectionRows( 'Expenditure' ) ) }
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td>{ __( 'Net Surplus / (Deficit)', 'wincobank-dashboard' ) }</td>
                                    { years.map( ( y ) => {
            ...
            { ! loading && ! data && ! error && (
                <p className="wb-empty">{ __( 'Select comparison years and click Load Comparison.', 'wincobank-dashboard' ) }</p>
            ) }
        </div>
    );
}
